...
# PPTX视觉管线专用prompt（输入是逐页视觉描述，不是原始文字）
PPTX_VISION_INTERPRET_PROMPT = """你是一个文档理解助手。以下是一份PPT演示文稿的逐页视觉识别结果，每页由AI视觉模型描述。

请对内容进行分析，生成结构化的内容概述。

要求：
1. 先生成3-5句话的全文摘要，概括这份PPT的核心主题、关键数据和主要结论
2. 按PPT的内容逻辑（而非严格按页码）分段概述，将相关页面合并，每段1-3句话
3. 保留所有关键数据点（数字、百分比、金额、对比数据等）
4. 忽略装饰性描述（如"背景为蓝色"、"左侧有几何图案"等），只关注实质内容
5. 使用中文输出，信息密度高，不说废话
...
_PROMPT_MAP = {
    ...
    "pptx_vision": PPTX_VISION_INTERPRET_PROMPT,
